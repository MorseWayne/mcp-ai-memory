...
```
步骤 1: 搜索已有记忆
→ 调用 search_memories(query="[项目名] 功能/设计/架构 相关内容")
→ 注意：query 必须包含项目名，格式为 "[项目名] 关键词"
...
步骤 2: 搜索相关记忆
→ 调用 search_memories(query="[项目名] 变更相关的功能/模块名")
→ 注意：query 必须包含项目名，避免影响其他项目的记忆
...
> **重要**：搜索时**必须**在 query 中包含项目名，否则会返回所有项目的记忆！
...
```
✅ "[mcp-ai-memory] 核心功能"
✅ "[mcp-ai-memory] API 接口设计"
✅ "[mcp-ai-memory] 配置参数说明"
✅ "[my-project] 用户认证流程"
```
...
5. **用户确认**：重要操作前可询问用户确认
6. **搜索带项目名**：搜索时必须在 query 中包含 `[项目名]`，否则会返回全量记忆
